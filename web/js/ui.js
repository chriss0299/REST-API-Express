...
export function mostraUtenti(utenti, contenitore, callbacks, utenteLoggato) {
  ...
  utenti.forEach((utente) => {
    ...
    const isOwner = utenteLoggato && utenteLoggato.id === utente.id;
    ...
    // Il tasto Post è sempre visibile
    let htmlAzioni = `<button class="btn-primario" data-azione="vedi-post">Post</button>`;

    // Mostra Modifica/Elimina solo se ha i permessi
    if (isAdmin || isOwner) {
      htmlAzioni += `
    ...
    azioni.innerHTML = htmlAzioni;
    card.appendChild(azioni);

    // Event Listeners con protezione (?.)
    card
...
export function mostraCommenti(
  commenti,
  contenitore,
  callbacks,
  utenteLoggato,
  ...
  commenti.forEach((c) => {
    const puoEliminare =
      utenteLoggato &&
      (utenteLoggato.id === c.userId || utenteLoggato.ruolo === "admin");
    const card = document.createElement("div");
